...
| 5  | Designing business logic in a microservice architecture | Reading | — | Conceptual walkthrough started in Claude Code (transaction script, DDD aggregates, domain events) — not yet read from the book itself; continuing in Claude Chat |
...
- **Chapter**: 5 — Designing business logic in a microservice architecture — Reading (Ch. 4 fully Done). A Claude Code session gave a first-pass conceptual walkthrough of the three patterns for this chapter — transaction script, DDD aggregates, domain events — mapped onto this codebase's actual code (see "Concept understanding" below for the specifics), but this was Claude's own explanation, not yet the book's own text. Read the book chapter itself next (in Claude Chat or wherever), then come back to build.
- **Status**: Mid-Ch.5 reading — no implementation started yet. Leading candidates once reading resumes: refactor `Ticket` (kitchen-service) and/or `Order` (order-service) from their current near-anemic shape into real aggregates with enforced state-transition invariants and self-generated domain events.
...
- Ch. 5 (partial, Claude's explanation — not yet the book's own text): transaction script (procedural service classes over near-anemic entities — this is what `TicketService`/`SagaJoinService` already look like today); DDD aggregates (business rules move into the entity itself, e.g. `Ticket.markCancelled()` would enforce "can't cancel an already-accepted ticket" internally instead of the caller trusting no invalid transition slips through); aggregate = single-transaction consistency boundary, which is *why* sagas (Ch.4) are needed at all — cross-aggregate operations can't share one ACID transaction; domain events as a natural byproduct of a state-changing aggregate method (returning events from `markCancelled()`) rather than a separately hand-written line of event-construction code that's easy to forget (contrast with how outbox events are constructed today across `TicketService`/`ConsumerVerificationService`/`SagaJoinService`).

### Needs more depth
- Ch. 5: everything — only had Claude's own explanation so far, not the book's actual treatment. Read the chapter itself before treating any of the above as settled.

### Open questions
- Ch. 5: which aggregate to refactor first when implementation starts — `Ticket` (kitchen-service) and `Order` (order-service) were both flagged as candidates (see "Current position" above), not yet decided.
...
---
*Last updated: 2026-07-18 — Ch. 5 reading started*
