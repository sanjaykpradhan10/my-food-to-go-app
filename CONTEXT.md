...
- **Status**: Implementing (Create Order saga, choreography style, implemented and manually verified end-to-end; orchestration style planned as a follow-up exercise to compare directly)
...
| ftgo-order-service | Ch. 2–4 | REST call + circuit breaker to restaurant-service (discovered via Eureka); publishes via outbox (polling or CDC); saga participant (creates in APPROVAL_PENDING, transitions to APPROVED/REJECTED via OrderSagaService) | Owns Order domain model (now with consumerId); POST /orders calls restaurant-service via a @LoadBalanced RestClient wrapped in a Resilience4j circuit breaker; persists Order + OrderCreated outbox row in one transaction; three Kafka listeners (consumer.events/kitchen.events/accounting.events) drive the saga's terminal transition |
...
- [ ] Saga — orchestration (Ch. 4) — planned follow-up, same Create Order use case, to compare directly against choreography
