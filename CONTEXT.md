...
- 2026-07-18 · Claude Code · Upgraded Spring Boot 3.5.3 → 3.5.16 (the final 3.5.x release — the line reached open-source end-of-life on 2026-06-30) and order-service's Spring Cloud BOM 2025.0.0 → 2025.0.3 (latest patch compatible with Boot 3.5.x; Spring Cloud 2025.1.x requires Boot 4.0). Checked every documented 3.5.x behavioral change (TaskExecutor bean renaming, heapdump endpoint default, stricter `.enabled` boolean parsing, profile-name restrictions, Redis/Prometheus/Groovy property renames, docker-compose SSL fixes) against this codebase — none apply (no actuator, no `taskExecutor` bean, no `.enabled` properties, no `@Profile` usage, no Spring Boot docker-compose integration), so this was a pure version bump with no code changes. Verified via full `./gradlew clean build` and a Docker e2e pass reusing the same four scenarios as every other dependency-touching change in this project (choreography happy path, orchestration happy path, one compensation case, redelivery/idempotency) — all passed unchanged.
