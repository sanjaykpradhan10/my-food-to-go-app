...
- **Status**: `Order` aggregate done (sub-project 1 of 3 for the cancel/revise scope). Two sequenced sub-projects remain, each its own future session: **Cancel Order saga** (sub-project 2) and **Revise Order saga** (sub-project 3) — both need real cross-service saga wiring (kitchen ticket adjustment, accounting refund/re-authorization) in both saga modes to resolve the `CANCEL_PENDING`/`REVISION_PENDING` states the aggregate can now enter but nothing yet resolves.
...
---
*Last updated: 2026-07-21 — Ch. 5 `Order` aggregate implemented (create/cancel/revise state machine); Cancel Order saga and Revise Order saga remain*
