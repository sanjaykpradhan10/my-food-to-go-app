...
| 13 | Refactoring to microservices | Done — conceptual only, no implementation. This chapter describes migrating an *existing monolith* to microservices (strangler application, implementing new features as services, extracting business capabilities, integration glue, anti-corruption layer, sequencing service extraction to avoid compensating transactions in the monolith, dual-mode session/JWT auth); this project has no monolith to migrate (it was built as microservices from Ch.1 onward), so there's nothing to implement — see `CONTEXT.md`'s Concept understanding section for the patterns learned |
