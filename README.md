...
| 12 | Deploying microservices | §12.1–12.3 (deployment patterns, Docker, Docker Compose — sub-project A) done, see [`docs/CH12-DEPLOYMENT.md`](docs/CH12-DEPLOYMENT.md). §12.4 (Kubernetes) sub-project B1 (core deployment) done — the entire `compose.yml` stack redeployed to a local `kind` cluster via a Helm chart at `k8s/ftgo/`, verified by the `ftgo-end-to-end-test` suite against a Kubernetes profile; see `docs/ARCHITECTURE.md`'s Kubernetes deployment section. B2 (zero-downtime rolling deployment, §12.4.4) done — see docs/ARCHITECTURE.md; B3a (Linkerd service mesh, auto-mTLS) done — see docs/ARCHITECTURE.md |
