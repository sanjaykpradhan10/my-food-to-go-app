...
This starts MySQL (port 3306), Zookeeper (2181), Kafka (9092), and the observability stack: Prometheus (9090), Grafana (3000), Tempo (3200/4318), Elasticsearch (9200), Logstash (5044), Kibana (5601), and GlitchTip (8000). On first boot, MySQL creates all service schemas and grants the `ftgo` user access to each.
...
| 11 | Developing production-ready services | In progress — §11.1 (authenticating/authorizing requests) sub-project 1 done: new `ftgo-authorization-server` module issuing JWTs (custom resource-owner-password grant); both gateways and all 7 business services (order/kitchen/restaurant/accounting/delivery/consumer/order-history) converted to OAuth2 resource servers with `@PreAuthorize` role requirements; order-service also gained an instance-based ACL (a consumer can only view their own order) with `consumerId` derived from the JWT rather than the request body. Sub-project 2 done: `client_credentials` grant added for service-to-service calls (order-service's internal calls to restaurant/kitchen/accounting/delivery-service now carry a `SERVICE`-role bearer token via `ServiceTokenClient`); accounting-service's authorization-lookup endpoint widened to accept `ADMIN` or `SERVICE`. §11.2 (externalized configuration) done: Spring Cloud Config Server (`ftgo-config-server`, port 8888), git-backed (`config-repo/`), serving centralized defaults plus per-service overrides (5 outbox services); non-blocking "optional" startup contract (`spring.cloud.config.fail-fast: false`); live refresh of `outbox.poll-fixed-delay-ms` via `POST /actuator/refresh` on the 5 outbox services; other properties (Kafka/Eureka/JWT settings) require full restart. §11.3 (observability) sub-project "health checks" (§11.3.1) done; sub-project "application metrics" (§11.3.4) done: Micrometer + Prometheus (`/actuator/prometheus`, custom business counters on 7 services) plus a Grafana dashboard, both as `compose.yml` services with alert rules. §11.3 sub-project "distributed tracing" (§11.3.3) done: Micrometer Tracing + OpenTelemetry (`micrometer-tracing-bridge-otel`/`opentelemetry-exporter-otlp`, replacing the deprecated Sleuth/Zipkin stack) on all 9 services, exporting via OTLP/HTTP to a new `tempo` compose service (Grafana Tempo) at 100% sampling; automatic HTTP/JDBC span instrumentation plus explicit Kafka observation properties on the 6 Kafka-consuming/producing services (order/kitchen/accounting/delivery/consumer/order-history-service; `ftgo-order-history-service`'s custom listener factory and `ftgo-common`'s custom `KafkaTemplate` bean both needed an extra explicit `setObservationEnabled(true)` call since a hand-built Kafka bean bypasses Boot's property-driven autoconfiguration) and explicit Reactor context-propagation enablement in `ftgo-gateway-common` for both gateways; verified end-to-end by a Cucumber scenario polling Tempo's search API for a trace spanning multiple services including a Kafka-only-reachable one. §11.3 sub-project "log aggregation" (§11.3.2) done: ELK stack (Elasticsearch, Logstash, Kibana) + Filebeat in `compose.yml`; all 9 services log structured JSON to stdout via `logstash-logback-encoder`, with `traceId`/`spanId` copied from the Micrometer Tracing MDC; Filebeat autodiscovery ships container stdout to Logstash, which parses JSON and indexes into Elasticsearch as `ftgo-logs-*`, searchable and correlatable by `traceId` in Kibana (index pattern auto-provisioned on startup). §11.3 sub-project "exception tracking" (§11.3.5) done: GlitchTip (Postgres + Redis-backed, `glitchtip/glitchtip:v4.2.9`) added to `compose.yml`; a one-shot `glitchtip-provisioner` container bootstraps an org/project/DSN via GlitchTip's Django management shell and writes it to a shared Docker volume every service container's entrypoint sources at startup; `sentry-spring-boot-starter-jakarta` on all 9 services auto-captures only uncaught exceptions (existing `@ExceptionHandler`s stay untouched); NOT correlated with Tempo/Kibana `traceId`s (no OpenTelemetry integration wired in — the Sentry SDK's trace id is independent); verified by a Cucumber scenario polling GlitchTip's issues API after hitting an ADMIN-gated diagnostic endpoint that deliberately throws. §11.3's remaining pattern (audit logging, §11.3.6) not started |
| 12–13 | … | Not started |
