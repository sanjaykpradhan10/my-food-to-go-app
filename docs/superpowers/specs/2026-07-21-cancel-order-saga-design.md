...
4. **`SagaReply` gains a `sagaType` field** (`"CreateOrder"` / `"CancelOrder"`), so `order-service`'s shared `OrchestratorReplyListener` can route a reply to the correct orchestrator now that two saga types share the same `orderId` keyspace. For most participant handler methods, the literal is a constant supplied at the `publishReply(...)` call site, since that method only ever serves one saga. **`KitchenCommand` (the one command record shared by both sagas) is the exception**: `CreateOrderSagaOrchestrator.sendCancelTicket` (compensation) and the new `CancelOrderSagaOrchestrator.start` both send a `"CancelTicket"` command, handled by the same `TicketService.handleCancelTicketCommand`. That handler cannot statically know which saga a given `CancelTicket` command belongs to — so `KitchenCommand` itself gains the `sagaType` field, set by whichever orchestrator sends the command, and `handleCancelTicketCommand` echoes it back unchanged in its reply. (Discovered during plan-writing — the original brainstorm assumed no command records needed changes, which holds for every command except this one shared one.) Without this, a Create-saga compensation `TicketCancelled` reply could get mis-tagged and routed to `CancelOrderSagaOrchestrator`, which would issue a spurious `ReverseAuthorization` for an order accounting may never have authorized.
...
`SagaReply(eventId, participant, eventType, orderId, reason)` gains a 6th field, `sagaType`, in all 4 services' independent copies of the record (order/kitchen/consumer/accounting). Every existing `publishReply(...)` call site across all 4 services is updated to pass `sagaType="CreateOrder"` (the only saga type today) — except `handleCancelTicketCommand`, which passes through whatever `sagaType` its inbound `KitchenCommand` carried (see scope decision 4). The new `ReverseAuthorization` reply site always passes `sagaType="CancelOrder"` (that command only ever exists for this saga).

`KitchenCommand(eventId, commandType, orderId, totalQuantity)` gains a 5th field, `sagaType`. Every existing sender (`CreateOrderSagaOrchestrator`'s `"CreateTicket"`/`"ConfirmTicket"`/`"CancelTicket"` sends) passes `"CreateOrder"`; the new `CancelOrderSagaOrchestrator`'s `"CancelTicket"` send passes `"CancelOrder"`. `TicketService.handleCreateTicketCommand`/`handleConfirmTicketCommand` ignore the field (always `"CreateOrder"` in practice); `handleCancelTicketCommand` reads it and echoes it into its reply's `sagaType`.
