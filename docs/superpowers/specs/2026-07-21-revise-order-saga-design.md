...
6. **Kitchen's revision-legal states mirror `cancel()`'s existing state boundary** — legal from `CREATE_PENDING`/`AWAITING_ACCEPTANCE`/`ACCEPTED`/`PREPARING`, illegal from `READY_FOR_PICKUP`/`PICKED_UP`/`CANCELLED` (once a ticket is ready or picked up, its quantity can no longer change). Unlike `cancel()`, revision doesn't get a two-tier exception split (`TicketCannotBeCancelledException` vs. generic) — that distinction exists in the book's own cancel example specifically, not as a general pattern this project extends to every guarded transition, so all illegal-state cases throw the generic `UnsupportedStateTransitionException`. `undoRevision()` has no state restriction beyond `CANCELLED` — reverting to a previously-valid quantity is always legal from any other state, since it's undoing a change this same saga just made.
...
Order.revise(revision) -> REVISION_PENDING, publishes OrderRevisionProposedEvent
  ...
  accounting-service's KitchenEventListener (extended RELEVANT_EVENT_TYPES): reacts to "TicketQuantityRevised" only
    -> AuthorizationReviseService.reviseForChoreography(eventId, orderId, newTotalQuantity)
       -> isAuthorized(newTotalQuantity)?
          ...
          no  -> AuthorizationRevisionRejectedEvent -> accounting.events ("AuthorizationRevisionRejected") — Authorization unchanged
  order-service:
    ...
    AccountingEventListener (extended):
      ...
      "AuthorizationRevisionRejected" -> OrderReviseSagaService.compensateRevision(orderId, eventId)
                                          -> publishes a new "OrderRevisionCompensationRequested" message to order.events,
                                             carrying Order's still-untouched pre-revision lineItems (reusing the existing
                                             lineItems wire field) -- NOT an OrderDomainEvent, since Order itself doesn't
                                             transition here (it stays REVISION_PENDING); a direct outbox write via a new
                                             OrderDomainEventPublisher.publishRevisionCompensationRequested(order, eventId)
                                             method, mirroring how publishOrderCreated already bypasses the OrderDomainEvent
                                             sealed interface for a similar one-off case
  kitchen-service's OrderEventListener: reacts to "OrderRevisionCompensationRequested" (distinct from "OrderRevisionRejected",
                                         which fires only on the terminal state transition and must NOT trigger an undo)
    -> TicketService.handleOrderRevisionRejected(eventId, orderId, originalTotalQuantity)
       -- originalTotalQuantity computed by kitchen by summing the message's lineItems, same as it already does for
          "OrderCreated"/"OrderRevisionProposed"
...
- New method `reviseQuantity(int newTotalQuantity)`: legal from `CREATE_PENDING`/`AWAITING_ACCEPTANCE`/`ACCEPTED`/`PREPARING` (same boundary as `cancel()` — scope decision 6); illegal from `READY_FOR_PICKUP`/`PICKED_UP`/`CANCELLED`, all uniformly `UnsupportedStateTransitionException` (no two-tier exception split — see scope decision 6). Sets `totalQuantity`, returns `TicketQuantityRevisedEvent(orderId, newTotalQuantity)`. Rejection (`TicketRevisionRejectedEvent(orderId, reason)`) is not thrown by the aggregate at all — `TicketService` decides via `isWithinCapacity` before ever calling this method, same as `createTicket()`.
- New method `undoRevision(int originalTotalQuantity)`: always legal regardless of state except `CANCELLED` (`UnsupportedStateTransitionException` — nothing to undo on a cancelled ticket, this shouldn't happen in practice since a revision can't have been in flight against an already-cancelled ticket, but guarded defensively). Sets `totalQuantity` back, returns `TicketRevisionUndoneEvent(orderId, originalTotalQuantity)`.
...
- `OrderEventListener` (kitchen-service) `eventType` switch gains `"OrderRevisionProposed"` and `"OrderRevisionCompensationRequested"` (not `"OrderRevisionRejected"` — see the choreography flow section on why these must stay distinct), alongside existing `"OrderCreated"`/`"OrderCancelled"`.
...
- New `AuthorizationReviseService`, mirroring `AuthorizationCancelService`'s existing choreography/command split exactly (the same split that fixed the orchestration-mode bug found during Cancel Order's Docker verification — see `docs/session-2026-07-21b.md`): `reviseForChoreography(eventId, orderId, newTotalQuantity)` publishes to `accounting.events`; `reviseForCommand(eventId, orderId, newTotalQuantity, sagaType)` publishes a `SagaReply` to `saga.replies`. Both check `isAuthorized(newTotalQuantity)` first and only call `authorization.reviseAuthorization()` if within threshold; on rejection, `Authorization` is left unchanged (no aggregate method call at all — matching `Ticket.reviseQuantity()`'s "service decides, aggregate doesn't self-reject" pattern).

## Order-service changes

- New `OrderReviseSagaService` (choreography), mirroring `OrderCancelSagaService`'s shape: `confirmRevision(orderId, eventId)` → `order.confirmRevision()`; `rejectRevision(orderId, eventId)` → `order.rejectRevision()`; `compensateRevision(orderId, eventId)` → calls the new `OrderDomainEventPublisher.publishRevisionCompensationRequested(order, eventId)` directly (does **not** call `Order.rejectRevision()`, since `Order` must stay `REVISION_PENDING` until kitchen's undo is confirmed); `finalizeRejectedRevision(orderId, eventId)` → `order.rejectRevision()` (called only after kitchen confirms `"TicketRevisionUndone"`). All dedup via `ProcessedEventRepository`, same pattern as every other saga participant.
- `Order.confirmRevision()` changes signature from `confirmRevision(OrderRevision revision)` (PR #12) to no-arg, reading the newly persisted `pendingRevisedLineItems` field instead — the caller here is an async reply handler in a different transaction than the original `/revise` request, so the `OrderRevision` object no longer exists to pass in. `OrderController.revise()` is unaffected (still calls `order.revise(new OrderRevision(...))`, unchanged). The only existing caller of the old signature is `OrderTest`, updated in the same task.
