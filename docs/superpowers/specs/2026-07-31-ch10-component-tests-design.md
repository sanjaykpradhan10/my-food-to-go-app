...
- **Kitchen / Accounting Services** — reached only asynchronously via Kafka saga commands/replies in orchestration mode. Stubbed with a small Java "saga participant stub" helper built directly on the `KafkaMessageVerifierSender`/`KafkaMessageVerifierReceiver` bridge from sub-project 1: it consumes the relevant saga command topic and publishes back a canned `SagaReply`, exactly mirroring the book's own `SagaParticipantStubManager` but reusing infrastructure already in this codebase instead of writing a new Kafka client from scratch. Delivery Service is not part of the Place Order flow's synchronous or saga-command path and is not stubbed.
