...
- `ftgo-kitchen-service`'s `OrderEventListener` (`ftgo-kitchen-service/.../infrastructure/OrderEventListener.java`): deserializes **every** `order.events` message directly as `OrderCreatedEvent` and calls `ticketService.handleOrderCreated(event)` unconditionally — **no `eventType` check**. This is the one other consumer of `order.events` in the system.
